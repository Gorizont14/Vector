...
void Pwm_Init(TPwm *p) //Main calc of 3-phase PWM, Control System call and ADC call
{
    ...
    // Contol of ePWM channel B comes from channel À:
    ...
    EPwm3Regs.TBCTL.bit.FREE_SOFT       = 1;                // Îñòàíîâêà øèì ïîñëå CTR = 0
   ...
   //Simultanious switch ON of all PWMs, all start to count from zero
    EALLOW;
    SysCtrlRegs.PCLKCR0.bit.TBCLKSYNC = 1; //âêëþ÷åíèå pwm clock
    EDIS;
}
